...
function initTimeline() {
    ...
    timelineInstance.on('click', (props) => {
        if (props.item) {
            ...
            if (item) showEventModal(item); // 传递对象，包含 event 属性
        }
    });
}
...
function initSearch() {
    ...
    function perform() {
        ...
        const all = [
            ...
            ...indexDataCache.festivals.map(f => ({...f, sType: 'festival'})),
            ...indexDataCache.proverbs.map(v => ({...v, sType: 'proverb'}))
        ];
        ...
        document.getElementById('modal-body').innerHTML = `<div class="search-results">${res.slice(0,15).map(i => `
            <div class="search-result-item" data-id="${i.id}" data-type="${i.sType}" style="cursor:pointer; padding:10px; border-bottom:1px solid #eee;">
                <h4>${i.name || i.title || i.event || i.phrase || '未命名'}</h4><p><small>类型: ${getTypeLabel(i.sType)}</small></p>
            </div>`).join('')}</div>`;
        ...
        modal.querySelectorAll('.search-result-item').forEach(el => el.addEventListener('click', () => {
            ...
            if (type === 'character') {
                ...
                const tl = timeline.find(t => t.id == id);
                if(tl) showEventModal(tl);
            } else {
                const item = (indexDataCache[type+'s'] || indexDataCache.items || []).find(d => d.id == id);
                if(item) showIndexItemDetail(id, type === 'poem' ? 'poems' : type === 'item' ? 'items' : type + 's');
            }
        }));
...
window.showIndexItemDetail = function(itemId, itemType) {
    const modal = document.getElementById('detail-modal');
    const realType = itemType.endsWith('s') ? itemType : itemType + 's';
    const list = indexDataCache[realType === 'characters' ? 'persons' : realType] || [];
    ...
    let html = (realType === 'persons' || realType === 'characters') ? `<p><strong>籍册：</strong>${item.group || '未入册'}</p><p><strong>身份：</strong>${item.identity}</p><hr><p>${item.description}</p>` :
               (realType === 'poems') ? `<div style="white-space:pre-wrap;text-align:center;font-family:serif;line-height:2;">${item.content}</div>` : 
...
function showEventModal(ev) {
    const modal = document.getElementById('detail-modal');
    // 同时兼容 events.json 的 title 和 timeline.json 的 event 字段
    document.getElementById('modal-title').textContent = ev.title || ev.event || "详情";
    
    let timeInfo = `第${ev.year || '?'}年`;
    if (ev.season) timeInfo += ` · ${ev.season}`;
    if (ev.chapter) timeInfo += ` · ${ev.chapter}`;

    document.getElementById('modal-body').innerHTML = `
        <p><strong>时间：</strong>${timeInfo}</p>
        <hr style="margin:10px 0; border:none; border-top:1px solid #eee;">
        <p style="line-height:1.6;">${ev.description || ev.content || '暂无详细描述'}</p>
...
function getTypeLabel(t) { return {character:'人物',event:'事件',timeline:'情节',poem:'诗词',item:'器物',festival:'节日',proverb:'俗语',main:'主要人物',major:'重要人物'}[t] || t; }
